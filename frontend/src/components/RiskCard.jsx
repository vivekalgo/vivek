...
export default function RiskCard({ clause, index }) {
    ...
    const CollapsibleSection = ({ title, icon, isExpanded, onToggle, children, bgColor = "bg-slate-50", borderColor = "border-slate-200" }) => (
        <div className={`${bgColor} border-2 ${borderColor} rounded-xl overflow-hidden transition-all duration-200`}>
            <button
                onClick={onToggle}
    ...
    return (
        <Card
            ...
            id={`clause-${index}`}
            hover
            className={`mb-4 transition-all duration-300 border-l-4 ${highlighted ? 'ring-4 ring-primary-500 shadow-2xl clause-highlight border-l-primary-500' :
            ...
            <div className="flex items-start justify-between mb-4">
                <div className="flex-1">
                    <div className="flex items-center gap-2 mb-3">
                        <div className={`w-8 h-8 rounded-lg bg-gradient-to-br ${getRiskColor(clause.risk_level)} flex items-center justify-center`}>
